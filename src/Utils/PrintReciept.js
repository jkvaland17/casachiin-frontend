...
export const DownloadScreeningDeptResult = async (
  token,
  advertisementId,
  departmentId,
  committeeId = "",
  ...
  const dto = `advertisementId=${advertisementId}&departmentId=${departmentId}&committeeId=${committeeId}&screeningConfigId=${screeninConfigid}`;

  await toast.promise(
    Dataservice.GetScreeningReport(dto, token).then((response) => {
      if (response?.data) {
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "Screening Report.pdf");
        document.body.appendChild(link);
        link.click();
        link.parentNode.removeChild(link);
      }
