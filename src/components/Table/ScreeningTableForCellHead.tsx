...
const ScreeningTableForCellHead = ({
  subitems,
  updateDocs,
  setTempData,
  GetDataDocument,
  ...
  const itemsWithIndex = subitems?.docs.map((item: any, index: any) => ({
    ...item,
    index,
  }));

  const tableColumnDoc =
    ScreenAllData?.data?.isMultiStage && adminType?.value === "CellAdmin"
      ? [
          { name: "Sr No.", uid: "no" },
          { name: "HOD Status", uid: "HODstatus" },
          { name: "Status 1", uid: "status1" },
          { name: "Status 2", uid: "status2" },
          { name: "Status", uid: "status" },

          { name: "Document", uid: "document" },
          { name: "HOD Remark", uid: "HODremarks" },
        ...
        case "status":
          return (
            <div className="flex justify-center">
              {dataRow?.adminFinalDocumentStatus ? (
                <Chip
                  variant="flat"
                  color={getChipColor(dataRow?.adminFinalDocumentStatus)}
                >
                  {dataRow?.adminFinalDocumentStatus}
                </Chip>
              ) : (
          ...
          return (
            <div className="flex justify-center">
              <Popover placement="top">
                <PopoverTrigger>
                  <p className="truncate w-[200px] hover:cursor-pointer">
                    {dataRow?.adminFinalDocumentRemark || "-"}
                  </p>
                </PopoverTrigger>
                <PopoverContent>
                  <p className="w-[250px] p-2">
...
export default ScreeningTableForCellHead;
