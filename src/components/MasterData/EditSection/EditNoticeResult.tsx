...
const EditNoticeResult: React.FC<any> = ({ slug }) => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      setLoading(true);
      const { data: response, error } = (await CallUpdateMasterData(
        data,
      )) as any;
  ...
  return (
    <>
      ...
      <Card className="max-w-full px-3 rounded-lg bg-[#fdfdfd]">
        <CardHeader className="flex gap-3">
          <div className="flex w-full items-center gap-x-3 mt-3 text-2xl justify-between">
            <p>Add Notice</p>
            <Button
              as={Link}
              href="/admin/master/master-data/notice"
              color="primary"
              variant="flat"
...
export default EditNoticeResult;
