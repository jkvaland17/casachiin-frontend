...
const UploadExcel: React.FC<any> = ({
  label,
  CallUploadExcelApi,
  FormDataExcel,
  sampleExcelUrl,
  ...
  const validateData = (data: any) => {
    const requiredFields = ["Code", "Reason", "Status", "SelectedType"];
  ...
  const onSubmit = async () => {
    try {
      if (!isRead && !validateData(excelData)) {
        toast.error("Valid status not selected or invalid excel data.");
        return;
      }
      setIsLoading(true);
      let responseData;
      if (isRead) {
        ...
        formDataExcel.append("advertisementId", FormDataExcel?.advertisementId);
        formDataExcel.append("statusCode", FormDataExcel?.statusCode);
        formDataExcel.append("startDate", FormDataExcel?.startDate);
        formDataExcel.append("endDate", FormDataExcel?.endDate);
        formDataExcel.append("remark", remark);
        responseData = (await CallUploadExcelApi(formDataExcel)) as any;
      } else {
        const dto = {
          advertisementId: FormDataExcel?.advertisementId,
          statusCodeData: excelData,
          title,
        };
        responseData = (await CallUploadExcelApi(dto)) as any;
      }
  ...
  const headers = Object.keys(
    excelData?.reduce((acc: any, obj: any) => {
      return { ...acc, ...obj };
    }, {}),
  );

  const handleRadioChange = (value: any, index: number) => {
    setExcelData((prevData: any) =>
      prevData.map((item: any, i: number) =>
        i === index ? { ...item, SelectedType: value } : item,
      ),
    );
  };
  ...
  return (
    <>
      ...
      <div className="my-3">
        {isRead && excelData?.length > 0 ? (
          <DynamicTable data={excelData} />
        ) : (
          <>
            {excelData?.length > 0 && (
              <div className="my-3">
                ...
                >
                  ...
                  <TableBody emptyContent={"No data found."}>
                    {excelData?.map((item: any, index: number) => (
                      <TableRow className="rounded-xl" key={index}>
                        {headers.map((header) => (
                            ...
                            {item[header] === "Status" ? (
                              <Select
                                ...
                                items={[
                                  ...
                                  {
                                    value: "Category Certificate",
                                    name: "Category Certificate",
                                  },
                                  {
                                    value: "PWBD Certificate",
                                    name: "PWBD Certificate",
                                  },
                                  { value: "Application", name: "Application" },
                                  { value: "Only Reason", name: "Only Reason" },
                                ]}
                                value={item?.SelectedType}
                                selectedKeys={[item?.SelectedType]}
                                onSelectionChange={(e: any) => {
                                  handleRadioChange(Array.from(e)[0], index);
                                }}
      ...
      <div className="my-3 w-full flex items-center justify-center mt-6">
        {excelData?.length === 0 ? (
          <Button
            isDisabled={excelData?.length > 0}
            onClick={() => downloadExcel(sampleExcelUrl)}
            color="primary"
            className="px-20"
            radius="sm"
          >
      ...
      >
        <ModalContent>
          {() => (
            <>
              <ModalHeader className="mb-0 pb-0">Remark</ModalHeader>
              <ModalBody>
                <Textarea
                  radius="sm"
